...
> 请按里程碑顺序直接开始搭建代码，M0-M3 完成后暂停等待人工验收，再继续后面的内容。
...
**明确不做（v1 之外）：**
- RPG Maker XP / VX（老版本 Marshal，类定义和 VX Ace 有差异，放到 v1.1）
- RPG Maker 2000/2003（LcfLMU/LcfLDB 格式，完全不同的解析器）
- Unity、Ren'Py、KiriKiri 等非 RPG Maker 引擎
- 运行时 hook / 内存注入模式
...
project/
  ...
  codec/
    ...
    rvdata2_codec.py                # rubymarshal 封装 + zlib 处理
  translate/
    glossary.py                      # 术语抽取
...
**数据库文件字段白名单**：每个数据库 JSON 是数组，元素里抓 `name`、`description`、`note`
（note 混杂插件配置和剧情备注，先抓出来，翻译前过一遍启发式过滤——整段是 `<tag:value>`
格式就跳过）、`message1`~`message4`（Skills 战斗提示语）。
...
- **M7（之后再说）**：RPG Maker XP/VX 支持、术语表更完善的审核界面、可选的运行时 hook 模式。
